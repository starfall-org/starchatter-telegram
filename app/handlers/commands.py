...
@Client.on_message(filters.command("group_menu"))  # type: ignore
async def group_menu(client: Client, message: types.Message):
    ...
    if not group:
        await db.add(TelegramGroup(id=message.chat.id,
                                   title=message.chat.title or "", username=message.chat.username))
        await db.commit()
        group = await db.get(TelegramGroup, id=message.chat.id)

    if not user:
        await db.add(TelegramUser(id=message.from_user.id,
    ...
    keyboard_markup = types.InlineKeyboardMarkup([[types.InlineKeyboardButton(
        text=("Disable" if group.disable_chatbot else "Enable") + " Chatbot", callback_data="_chatbot"
    ), types.InlineKeyboardButton(
        text=("Disable" if group.disable_anti_spam else "Enable") + " Anti-Spam", callback_data="_anti_spam"
    )], [
        types.InlineKeyboardButton(
            text="Goodbye", callback_data="_goodbye"
        )
    ]])
...
@Client.on_message(filters.command("menu") & filters.user(OWNER_ID)  # type: ignore
                   )
async def admin_menu(_: Client, message: types.Message):
    await message.reply_chat_action(enums.ChatAction.TYPING)
    keyboard_markup = types.InlineKeyboardMarkup([[types.InlineKeyboardButton(
        text="LLM Config", callback_data="config"
    ),
        types.InlineKeyboardButton(
        text="Edit", callback_data="edit_config"
    )
    ], [
        types.InlineKeyboardButton(
